...
class TermSource(StrictModel):
    """A canonical dictionary inside the target's translations directory."""

    file: str
    path: list[str] = Field(default_factory=list)

    _file = field_validator("file")(output_path)
...
def load_project(path: Path = DEFAULT_CONFIG) -> Project:
    ...
    cache = resolve(settings.cache) / settings.id
    backends = {}
    ...
    for code, values in config.targets.items():
        validate_locations((source.file, source.path) for source in values.term_sources)
